...
- **tests**: Match the executable in the subprocess guard, not the line
  ([`4057ac5`](https://github.com/datapointchris/ypl/commit/4057ac57db099fc713dcfe2a9d54e266a9e05b40))

The autouse guard joined the whole command into one string and asked whether any forbidden binary
  appeared anywhere in it. Temp paths are part of that string and the suite does not choose them:
  mkdtemp produced /tmp/tmpv3_xagop/, "tmpv3" contains "mpv", and an editor test failed as though it
  had launched a video player.

About one run in sixty, and it lands on whichever commit is unlucky — this one surfaced on a CI
  change that touched two trigger lines, which is the worst possible place to spend debugging time.

Every call site puts the binary at argv[0] (`[binary_path(), *arguments]`), so matching the basename
  of argv[0] is both stricter and exact. The guard gets its own tests: it is autouse and the only
  thing between the suite and the signed-in account, so a misfire blames an unrelated test and an
  under-fire is silent.

### Chores

- **lint**: Ignore the generated CHANGELOG.md
  ([`176cc71`](https://github.com/datapointchris/ypl/commit/176cc71f4a57e0842f88474f857aaee0218f6c75))
...
- Drop the push trigger the release run already covers
  ([`93e170d`](https://github.com/datapointchris/ypl/commit/93e170db9795ee5580b4349ad0be0235fb2b2c6c))
...
Now a fleet rule in standards/python.md.
...
This departs from the fleet's no-args-shows-help rule in `standards/cli-design.md`, under the
...
The trailer under a truncated failure list said how many more there were and left the reader to work
  out where. `standards/cli-design.md` settles that one: no remainder counts, the trailer is
